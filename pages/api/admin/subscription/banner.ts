...
export default async function handler(
  ...
  const sectionName = "subscription/banner";

  if (req.method === "GET") {
    try {
      const config = await Promise.resolve(
        readConfig<SubscriptionBannerConfig>(sectionName)
      );
  ...
  if (req.method === "POST") {
    try {
      ...
      await Promise.resolve(writeConfig<SubscriptionBannerConfig>(sectionName, body));
