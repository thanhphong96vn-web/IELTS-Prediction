...
export default async function handler(
  ...
  if (req.method === "GET") {
    try {
      const config = await Promise.resolve(readConfig<SampleEssayBannerConfig>(sectionName));
  ...
  if (req.method === "POST") {
    try {
      ...
      await Promise.resolve(writeConfig<SampleEssayBannerConfig>(sectionName, body));
