...
export default async function handler(
  ...
  if (req.method === "GET") {
    try {
      const config = await Promise.resolve(readConfig<TestPlatformIntroConfig>(sectionName));
  ...
  if (req.method === "POST") {
    try {
      ...
      await Promise.resolve(writeConfig<TestPlatformIntroConfig>(sectionName, body));
