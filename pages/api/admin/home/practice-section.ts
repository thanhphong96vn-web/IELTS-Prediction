...
export default async function handler(
  ...
  const sectionName = "practice-section";

  if (req.method === "GET") {
    try {
      const config = await Promise.resolve(readConfig<PracticeSectionConfig>(sectionName));
  ...
  if (req.method === "POST") {
    try {
      ...
      const body = req.body as PracticeSectionConfig;
      await Promise.resolve(writeConfig<PracticeSectionConfig>(sectionName, body));
