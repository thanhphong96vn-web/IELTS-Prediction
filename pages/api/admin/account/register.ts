...
export default async function handler(
  ...
  const sectionName = "account/register";

  if (req.method === "GET") {
    try {
      const config = await Promise.resolve(readConfig<RegisterPageConfig>(sectionName));
  ...
  if (req.method === "POST") {
    try {
      ...
      const body = req.body as RegisterPageConfig;
      await Promise.resolve(writeConfig<RegisterPageConfig>(sectionName, body));
