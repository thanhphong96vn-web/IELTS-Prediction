...
export default async function handler(
  ...
  const sectionName = "account/login";

  if (req.method === "GET") {
    try {
      const config = await Promise.resolve(readConfig<LoginPageConfig>(sectionName));
  ...
  if (req.method === "POST") {
    try {
      ...
      const body = req.body as LoginPageConfig;
      await Promise.resolve(writeConfig<LoginPageConfig>(sectionName, body));
