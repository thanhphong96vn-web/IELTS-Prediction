...
export default async function handler(
  ...
  const sectionName = "ielts-exam-library/hero-banner";

  if (req.method === "GET") {
    try {
      const config = await Promise.resolve(readConfig<ExamLibraryHeroConfig>(sectionName));
  ...
  if (req.method === "POST") {
    try {
      ...
      const body = req.body as ExamLibraryHeroConfig;
      await Promise.resolve(writeConfig<ExamLibraryHeroConfig>(sectionName, body));
