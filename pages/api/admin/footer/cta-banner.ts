...
export default async function handler(
  ...
  const sectionName = "footer/cta-banner";

  if (req.method === "GET") {
    try {
      const config = await Promise.resolve(readConfig<FooterCtaBannerConfig>(sectionName));
  ...
  if (req.method === "POST") {
    try {
      ...
      const body = req.body as FooterCtaBannerConfig;
      await Promise.resolve(writeConfig<FooterCtaBannerConfig>(sectionName, body));
