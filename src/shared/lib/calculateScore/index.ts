...
const processMatchingQuestion = (
  ...
  if (!matchingData) {
    console.error(
      `>>> [Matching] Lỗi: Không tìm thấy matchingQuestion data tại answerIndex ${answerIndex}`,
    );
    return {
      details,
      correct,
  ...
  const layoutType = String(matchingData.layoutType || "standard")
    .trim()
    .toLowerCase();
...
const processMatrixQuestion = (
  ...
  if (!matrixData || !Array.isArray(matrixData.matrixItems)) {
    console.error(
      `>>> [Matrix] Lỗi: Không tìm thấy matrixItems data tại answerIndex ${answerIndex}`,
    );
    return {
      details,
      correct,
  ...
  matrixRows.forEach((row, rowIndex) => {
    ...
    const correctAnswerLetter = (row.correctCategoryLetter || "")
      .trim()
      .toLowerCase();

    // Lấy chữ cái user chọn (vd: "A", "B") từ object { 0: "A", 1: "B" }
    const userAnswerLetterRaw = userAnswerObject[rowIndex];
    const userAnswerLetter = String(userAnswerLetterRaw ?? "")
      .trim()
      .toLowerCase(); // vd: "a"
    ...
    if (userAnswerLetter) {
      const userOption = answerOptions.find(
        (opt: any) =>
          opt.categoryLetter.trim().toLowerCase() === userAnswerLetter,
      );
      userAnswerFullText = userOption
        ? userOption.categoryText
        : userAnswerLetter.toUpperCase();
    }

    // Tìm text đầy đủ của đáp án đúng
    const correctOption = answerOptions.find(
      (opt: any) =>
        opt.categoryLetter.trim().toLowerCase() === correctAnswerLetter,
    );
    correctAnswerFullText = correctOption
      ? correctOption.categoryText
      : correctAnswerLetter.toUpperCase();

    // So sánh chữ cái
    const isCorrect =
      userAnswerLetter === correctAnswerLetter && correctAnswerLetter !== "";

    if (isCorrect) {
      correct++;
    } else if (userAnswerLetter) {
      // Chỉ tính sai nếu user CÓ chọn
      incorrect++;
    }
...
const processRadioQuestion = (
  ...
  subQuestions.forEach((subQ) => {
    if (currentAnswerIndex >= answers.length) {
      console.warn(
        `>>> [Radio] Cảnh báo: Thiếu câu trả lời cho sub-question tại index ${currentAnswerIndex}`,
      );
      details.push({
        correct: false,
        userAnswer: null,
        answer: subQ.options?.[subQ.correct ?? 0]?.content ?? "N/A",
      });
      currentAnswerIndex++;
      return;
    }
...
const processSelectQuestion = (
  ...
  if (total === 0) {
    console.warn(
      `>>> [Select] Cảnh báo: Không tìm thấy đáp án đúng dạng {} trong text câu hỏi.`,
    );
  }
  ...
  correctAnswersFromText.forEach((correctAnswerText) => {
    if (currentAnswerIndex >= answers.length) {
      console.warn(
        `>>> [Select] Cảnh báo: Thiếu câu trả lời cho select tại index ${currentAnswerIndex}`,
      );
      details.push({
        correct: false,
        userAnswer: null,
...
const processFillupQuestion = (
  ...
  if (correctAnswersFromText.length > 0) {
    ...
    correctAnswersFromText.forEach((correctWordWithOptions) => {
      if (currentAnswerIndex >= answers.length) {
        console.warn(
          `>>> [Fillup-Text] Cảnh báo: Thiếu câu trả lời cho fillup tại index ${currentAnswerIndex}`,
        );
        details.push({
          correct: false,
          userAnswer: null,
      ...
      const possibleCorrectAnswers = correctWordWithOptions
        .split("|")
      ...
      const isCorrect =
        userAnswer !== "" && possibleCorrectAnswers.includes(userAnswer);
    ...
    if (total === 0) {
      console.warn(
        `>>> [Fillup-Exp] Cảnh báo: Không tìm thấy đáp án đúng trong explanations.`,
      );
    }
    ...
    correctAnswersFromExplanations.forEach((exp) => {
      if (currentAnswerIndex >= answers.length) {
        console.warn(
          `>>> [Fillup-Exp] Cảnh báo: Thiếu câu trả lời cho fillup tại index ${currentAnswerIndex}`,
        );
      ...
      const possibleCorrectAnswers = (exp.content || "")
        .split("/")
        .map((w) => w.trim().toLowerCase());
      const isCorrect =
        userAnswer !== "" && possibleCorrectAnswers.includes(userAnswer);
...
const processCheckboxQuestion = (
  ...
  if (answerIndex >= answers.length) {
    console.warn(
      `>>> [Checkbox] Cảnh báo: Thiếu câu trả lời cho checkbox tại answerIndex ${answerIndex}`,
    );
    const correctOptions =
      question.list_of_options?.filter((opt) => opt.correct) || [];
  ...
  const getOptionText = (index: number): string | null =>
    allOptions[index]?.option || null;
  ...
  if (total === 1) {
    ...
    const correctChoiceIndex =
      correctAnswersIndices.length > 0 ? correctAnswersIndices[0] : undefined;
    const isSingleCorrect =
      userChoiceIndex !== undefined && userChoiceIndex === correctChoiceIndex;
    ...
    details.push({
      ...
      userAnswer:
        userChoiceIndex !== undefined ? getOptionText(userChoiceIndex) : null,
      answer:
        correctChoiceIndex !== undefined
          ? getOptionText(correctChoiceIndex)
          : "N/A",
    });
  } else {
    const missedCorrectChoices = correctAnswersIndices.filter(
      (idx) => !userAnswer.includes(idx),
    );
    const missedCorrectText = missedCorrectChoices
      .map(getOptionText)
      .filter(Boolean) as string[];

    userAnswer.forEach((userChoiceIndex) => {
      const isThisChoiceCorrect =
        correctAnswersIndices.includes(userChoiceIndex);
...
export const calculateScore = (
  ...
  if (!quizData?.quizFields?.passages) {
    console.error(
      ">>> Lỗi calculateScore: Dữ liệu quizData hoặc passages không hợp lệ.",
    );
    return {
      score: "0.0",
      details: {},
  ...
  const passagesToScore = quizData.quizFields.passages.filter((_, index) =>
    validTestPart.includes(index),
  );

  passagesToScore.forEach((passage, passageIndexInFiltered) => {
    const passageDetails: QuestionDetail[] = [];
    let passageTotalQuestions = 0;
    const originalPassageIndex = quizData.quizFields.passages.findIndex(
      (p) => p === passage,
    );
    ...
    (passage.questions || []).forEach((question) => {
      ...
      if (typeof question.type === "string") {
        ...
        console.error(
          `>>> Lỗi calculateScore: Không xác định được loại câu hỏi tại passage index ${originalPassageIndex}`,
        );
        return;
      }
      ...
      switch (questionType) {
        case "matching":
          result = processMatchingQuestion(
            question,
            answers,
            answerIndex,
          ...
          break;
        case "matrix":
          result = processMatrixQuestion(question, answers, answerIndex);
          break;
        case "checkbox":
          result = processCheckboxQuestion(question, answers, answerIndex);
          break;

        case "radio":
          result = processRadioQuestion(question, answers, answerIndex);
          break;
        case "select":
          result = processSelectQuestion(question, answers, answerIndex);
          break;
        case "fillup":
          result = processFillupQuestion(question, answers, answerIndex);
          break;

        default:
          console.warn(
            `>>> Cảnh báo calculateScore: Loại câu hỏi '${questionType}' chưa được hỗ trợ.`,
          );
          result = {
            details: [],
            correct: 0,
    ...
    details[originalPassageIndex] = {
      ...
      questionRange:
        passageTotalQuestions > 0
          ? `Question ${startQuestionIndex} to ${endQuestionIndex}`
          : "No questions",
  ...
  const scoreValue =
    totalQuestions > 0 ? (correctCount / totalQuestions) * 9 : 0;
  const roundedScore = (Math.round(scoreValue * 2) / 2).toFixed(1);
  const missedCount = totalQuestions - correctCount - incorrectCount;

  console.log(
    `>>> KẾT QUẢ TÍNH ĐIỂM CUỐI CÙNG (V10): Correct=${correctCount}, Incorrect=${incorrectCount}, Missed=${missedCount}, Total=${totalQuestions}, Score=${roundedScore}`,
  );
